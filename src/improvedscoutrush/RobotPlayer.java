...
//					loadData();
	...
	static void runGardener() throws GameActionException {
		...
		boolean water = false;
		int numScouts = rc.readBroadcast(5)-1;
		if(rc.readBroadcast(5)==1) numScouts = -1;
        int numTrees = 0;
		...
		while (true) {
			// Try/catch blocks stop unhandled exceptions, which cause your robot to explode
			try {
			    System.out.println(" **** numTrees: " + numTrees + "; numScouts: " + numScouts + " **** ");
				// Check for incoming bullets
				nearbyBullets = rc.senseNearbyBullets();
				...
				else if (buildType == 4 && (numScouts <= numTrees || numTrees==5 || (numTrees>0 && !canBuildTree))) { // buildAxis != null &&
                    if(rc.canBuildRobot(RobotType.SCOUT, dir)) {
                        rc.buildRobot(RobotType.SCOUT, dir);
                        numScouts++;
                        rc.broadcast(10, 1 - rc.readBroadcast(10));
                    }
                    else System.out.println(" **** CANNOT BUILD SCOUT ****");
				}
				...
				if((buildAxis != null || ( (!rc.isCircleOccupied(rc.getLocation().add(dir, 2.01f), 1)
				    ...
				    int addDir;
                    for(addDir = 60; addDir < 360; addDir += 60) {
						if(rc.canPlantTree(dir.rotateRightDegrees(addDir))) {
						    if(numTrees < numScouts) {
	...
	static int chooseProduction(boolean archon) throws GameActionException {
		...
		else if(!rush && bestRatio != 0 && rc.senseNearbyTrees(-1, Team.NEUTRAL).length > 0 && rc.readBroadcast(5) >= 2) bestRatio = 2;

		System.out.println("** bestRatio : " + bestRatio);
		System.out.println("** Gardener  : #=" + rc.readBroadcast(5) + "; /=" + armyRatios[0]);
		System.out.println("** Soldier   : #=" + rc.readBroadcast(6) + "; /=" + armyRatios[1]);
		System.out.println("** Lumberjack: #=" + rc.readBroadcast(7) + "; /=" + armyRatios[2]);
		System.out.println("** Tank      : #=" + rc.readBroadcast(8) + "; /=" + armyRatios[3]);
		System.out.println("** Scout     : #=" + rc.readBroadcast(9) + "; /=" + armyRatios[4]);
